...
class DailyReading(db.Model):
    ...
    closing_meter = db.Column(
        db.Float
    )

    litres_sold = db.Column(
        db.Float
    )
...
class CorrectionRequest(db.Model):
    ...
    reading_id = db.Column(
        db.Integer,
        db.ForeignKey("daily_reading.id"),
        nullable=False
    )

    old_closing = db.Column(
        db.Float,
        nullable=False
    )

    new_closing = db.Column(
        db.Float,
        nullable=False
    )
...
def purchase():
    ...
    if request.method == "POST":
        ...
        if (
            q <= 0
            or t.current_stock + q > t.capacity
        ):
